#extract tweets from tweetfile* for round2, for >1500 tweets topics and considering filtered tags
import time
import re
import datetime
import dateutil.tz
...
path_tweet_extracts = '/mnt/filer01/round2/StraightExtracts/'
...
tags_org = set()
fd = open("/twitterSimulations/tweet_text/tweet_text_check/tweets_selected_tags_round2_"+fwritenum+".txt","w")
fd_err = open("/twitterSimulations/tweet_text/tweet_text_check/error_dump_round2_"+fwritenum+".txt","w")
count = 0
for tweet_folder in folder_contents:
	...
	txtfile_contents = os.listdir(os.path.join(path_tweet_extracts,tweet_folder,'mnt'))
