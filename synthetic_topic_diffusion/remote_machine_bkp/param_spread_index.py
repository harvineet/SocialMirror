...
for line in fr:
	...
	while numTweets < t_limit:
		...
		for i in candidates:			# neighbours of infected nodes only
			...
			for j in nodes:
				if i in adj[j]: # check if j already in adj, as j is in nodes
					try:
						prob_node += math.log(1.0 - alpha*math.exp(-1*beta*(present_time-nodes_time[j]))/ node_nbh[i])
					except:
						id_not_found+=1
			prob_node = 1-math.exp(prob_node)
			if (random.random()<=prob_node): # use pre-stored eta values for random number between 0 and 1 with high precision
				nodes.add(i)
