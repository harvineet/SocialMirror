...
m = pickle.load(open( "/twitterSimulations/map_user_index.pickle", "rb" ) )
...
def load_coupling_pickle(tag):
	if isfile("/mnt/filer01/parametric_spread/coupling/eta_"+tag+".pickle"):
		return pickle.load(open( "/mnt/filer01/parametric_spread/coupling/eta_"+tag+".pickle", "rb" ) )
	else:
		eta = dict()
		pickle.dump(eta, open( "/mnt/filer01/parametric_spread/coupling/eta_"+tag+".pickle", "wb" ) )
		return eta
...
print "alpha", alpha
print "beta", beta
fr = open('/twitterSimulations/timeline_data/timeline_weng_sample', 'r')
...
def topicwiseMAEwrite(topicName , inputAlpha , inputBeta , MeanAbsError):
	...
	#Acquired Lock.
	with open("/mnt/filer01/parametric_spread/synthetic_spread_log/"+sim_run+"/"+topicName + "_MAE.txt" , 'a+') as topicWiseWriter: #Automatically closes the file as well. yay.
		topicWiseWriter.write( str(inputAlpha) + ',' + str(inputBeta) + ',' + str(MeanAbsError))
	topicwiseMAEwriteLock.release()
	#Released lock.


forEachTopicFlag = (specificTopic == None) #Tells us whether synth.diff needs to be run for every topic or not.
...
for line in fr:
	...
	if not forEachTopicFlag :
		if u[0]!=specificTopic:
			continue
	fd = open("/mnt/filer01/parametric_spread/synthetic_spread_log/"+sim_run+"/"+alpha+"_"+ beta + u[0] +".txt", 'a+') #A new file for every topic to allow for RefineSearch in gridSearch.py
