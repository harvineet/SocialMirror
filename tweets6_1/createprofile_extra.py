...
if folder_contents==['mnt']:
	print 'mnt in', folder
	if "tweets" in folder:
		txtfile = folder.split('.')[0]+".txt"; # changed in case of tweets21.t
		path = "/mnt/filer01/Crawl1_extracts/extrafile_extracts/"+folder+"/mnt/"+txtfile;
		file_paths.append(path)
	else:
		txtfile = os.listdir(folder+'/mnt/1')[0]
		path = "/mnt/filer01/Crawl1_extracts/extrafile_extracts/"+folder+"/mnt/1/"+txtfile;
		file_paths.append(path)
else:
	for txtfile in folder_contents:
		print "tweets_portion in", txtfile
		path = "/mnt/filer01/Crawl1_extracts/extrafile_extracts/"+folder+"/"+txtfile;
		file_paths.append(path)
# fu = io.open("/mnt/filer01/Crawl1_extracts/TweetExtracts/"+folder+"/userfile_"+folder+".txt",'w',encoding='utf8')
fe = io.open("/mnt/filer01/Crawl1_extracts/extrafile_extracts/"+folder+"/errorfile_"+folder+".txt",'w',encoding='utf8')
f1 = io.open("/mnt/filer01/Crawl1_extracts/extrafile_extracts/"+folder+"/tweetfile_"+folder+".txt",'w',encoding='utf8')
