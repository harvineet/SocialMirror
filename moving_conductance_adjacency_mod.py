...
for line in fr:
	line = line.rstrip()
	u = line.split(' ')
	fd=open("./conductance_plot/feature_"+u[0]+".csv",'w')
	fd.write("TweetNum,TagName,Conduct1,Conduct2,Conduct3,Conduct4,Conduct2d,Conductance,TimeFirst1000,Class\n")
	tweet_window = 100
	step_size=1
	t_limit = min(len(u),10100)
	for pt in range(1,t_limit,step_size):
		...
		if len(u) > 10000:
			...
			fdtemp.close()'''
			fd.write(str(pt)+','+str(u[0])+','+str(expose1)+','+str(expose2)+','+str(expose3)+','+str(expose4)+','+str(expose2d)+','+str(float(expose_num)/expose)+','+str(val)+',1\n')
		else:
			'''fdtemp = open('dumpnviral/'+u[0], 'w')
			for i in range(0, len(timestamp_col)):
				fdtemp.write(str(timestamp_col[i]) + ' ' + str(expose_col[i]) + '\n')
			fdtemp.close()'''
			fd.write(str(pt)+','+str(u[0])+','+str(expose1)+','+str(expose2)+','+str(expose3)+','+str(expose4)+','+str(expose2d)+','+str(float(expose_num)/expose)+','+str(val)+',0\n')
