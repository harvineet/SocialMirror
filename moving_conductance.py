#Python file to create the conductance features by taking a moving window of tweets
import json
import sys
import time
import datetime
...
for run in range(1,6):
	if run==1:
		...
		tweet_window = 25
		step_size=1
	fr = open('timeline_data/timeline_weng_tag_plot', 'r')
	for line in fr:
		line = line.rstrip()
		u = line.split(' ')
		adopters = set()
		fd=open("./conductance_plot/"+name+"/feature_"+u[0]+".csv",'w')
		fd.write("TweetNum,TagName,Conduct1,Conduct2,Conduct3,Conduct4,Conduct2d,Conductance,TimeTillWindow,AbsoluteTime,NumAdoptersWindow,TotalNumAdopters,Class\n")
		t_limit = min(len(u),20100)#10100#len(u)#12100
		for pt in range(1,t_limit,step_size):
			...
			exposureGA = 0
			selfInitGeo = 0
			for i in range(pt, len(u)):
				timestamp = int(u[i][0:u[i].index(',')])
				numTweets1 = i
				if(numTweets1 >= pt+tweet_window):
					break
				...
				if author not in nodes:
					...
					if comm[community] == 1:
						freshGeo = True
					adj_author = adj[author]
					len_adj_author = len(adj_author)
					if len_adj_author > 3000:
						heavy = heavy + 1
					expose = expose + len_adj_author
					expose_num = expose_num + len_adj_author
					booladopt = True
					for value in nodes:
					...
					if booladopt:
						...
						exposureGA = exposureGA + len_adj_author
				nodes.add(author)
				adopters.add(author)
			...
			last_tweet_window = min(len(u),pt+tweet_window) - 1
			val = int(u[last_tweet_window][0:u[last_tweet_window].index(',')]) - int(u[1][0:u[1].index(',')])
			abs_time = int(u[last_tweet_window][0:u[last_tweet_window].index(',')])
			...
			if len(u) > 10000:
				...
				fdtemp.close()'''
				fd.write(str(pt)+','+str(u[0])+','+str(expose1)+','+str(expose2)+','+str(expose3)+','+str(expose4)+','+str(expose2d)+','+str(float(expose_num)/expose)+','+str(val)+','+str(abs_time)+','+str(len(nodes))+','+str(len(adopters))+',1\n')
			else:
				'''fdtemp = open('dumpnviral/'+u[0], 'w')
				for i in range(0, len(timestamp_col)):
					fdtemp.write(str(timestamp_col[i]) + ' ' + str(expose_col[i]) + '\n')
				fdtemp.close()'''
				fd.write(str(pt)+','+str(u[0])+','+str(expose1)+','+str(expose2)+','+str(expose3)+','+str(expose4)+','+str(expose2d)+','+str(float(expose_num)/expose)+','+str(val)+','+str(abs_time)+','+str(len(nodes))+','+str(len(adopters))+',0\n')
			#print pt, float(expose_num)/expose
		fd.close()
	fr.close()
#fr1.close()
#fr2.close()
