...
def read_adoption_sequence(adoption_sequence_filename, start, end):
...
def get_sentences(adoption_sequence_filename,start_line_num,end_line_num,process_num):
	tag_count = 0
	for t,segment in read_adoption_sequence(adoption_sequence_filename,start_line_num,end_line_num):
...
def write_sentence(process_num,start,end):
	with open(out_dir+'/hashtagAdoptionSentences'+str(process_num)+'.txt','wb') as fd:
		start_time = datetime.datetime.now()
		sentences = get_sentences(adoption_sequence_filename,start,end,process_num)
		for s in sentences:
			fd.write(" ".join(s)+"\n")
			# count[len(s)]+=1
...
for s,e in ( (i,min(i+lines_per_process,NUM_LINES)) for i in xrange(0,NUM_LINES,lines_per_process) ):
	pool.apply_async(write_sentence, args=(process_num,s,e))
	process_num+=1
pool.close()
pool.join()