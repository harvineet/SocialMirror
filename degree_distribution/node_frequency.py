...
"""
in_dir = "/mnt/filer01/word2vec/degree_distribution/sentences_files/"
...
def count_sentence_file(file_num,process_num):
	...
	pickle.dump(count,open(out_dir+"/frequencyNodes_1hr_bfs"+str(file_num)+".pickle","wb"))
	pickle.dump(context_count,open(out_dir+"/frequencyContextLength_1hr_bfs"+str(file_num)+".pickle","wb"))
...
for file_num in range(0,num_files):
	presentc = pickle.load(open(out_dir+"/frequencyNodes_1hr_bfs"+str(file_num)+".pickle","rb"))
	presentcc = pickle.load(open(out_dir+"/frequencyContextLength_1hr_bfs"+str(file_num)+".pickle","rb"))
	for i in presentc:
		count[i]+=presentc[i]
	for i in presentcc:
		context_count[i]+=presentcc[i]
pickle.dump(count,open(out_dir+"/comb_frequencyNodes_1hr_bfs.pickle","wb"))
pickle.dump(context_count,open(out_dir+"/comb_frequencyContextLength_1hr_bfs.pickle","wb"))	
"""
#plot
count = pickle.load(open("comb_frequencyNodes_1hr_bfs.pickle","rb"))
node_freq = []
for i in count:
...
ccount = pickle.load(open("comb_frequencyContextLength_1hr_bfs.pickle","rb"))
clength_freq = []
for i in ccount:
