...
arr = ["/twitterSimulations/timeline_data/anirban/quito_fol.txt"]
...
m = dict()
fr = open("/twitterSimulations/timeline_data/anirban/map_info_quito.txt")
for line in fr:
	line = line.rstrip()
	u = line.split('\t')
	m[int(u[0])] = int(u[1])
...
print 'Graph Set\n'
num_com = [0]*22440
fr = open('/twitterSimulations/timeline_data/anirban/Infomap-0.15.6/output/quito_fol_infomap.tree', 'r')
fr.readline()
for line in fr:
	line = line.rstrip()
	u = line.split(' ')
	#print m[int(u[0])]
	try:
		#Store community of a user (mapped ID used)
		buckets[int(u[2][1:len(u[2])-1])] = int(u[0][0:u[0].find(':')])		
		#***#
		num_com[int(u[2][1:len(u[2])-1])] = num_com[int(u[2][1:len(u[2])-1])] + 1
	except:
		a = 1
fr.close()

for i in range(0, 1000000):
	if num_com[buckets[i]] < 3:
		buckets[i] = -1


print 'Location Read\n'

fr = open('timeline_weng_quito', 'r')
fr1 = open('retweeteds_weng_quito', 'r')
fr2 = open('user_mentions_weng_quito', 'r')
fd = open("feature_quito.csv", 'w')
fd.write("RatioSecondtoFirst,RatioSelfInitCommu,RatioCrossGeoEdges,#globalAdoptersFollowers,#globalAdopters,#heavyusers,Density,LargestSize,NumEdges,Conduct1,Conduct2,NumTweets,TimeFirst200,NoOfAdopters,Conductance,RatioOfSingletons,RatioOfConnectedComponents,InfectedCommunities,UsageEntropy,NumOfRT,NumOfMention,IntraRT,IntraMen,Class\n")
mention_line = "";
u2 = []
...
for line in fr:
	...
	exposureGA = 0
	selfInitGeo = 0
	#CODE TO FIND CONDUCTANCE OF GRAPH
	for i in range(1, len(u)):
		timestamp = int(u[i][0:u[i].index(',')])
		numTweets1 = i
		if(timestamp - int(u[1][0:u[1].index(',')]) > 9000 and numTweets1 > 50):
			break
		if(timestamp - int(u[1][0:u[1].index(',')]) < 9000):
			numTweets = i
	...
	initSet(len(nodes))
	singleton = len(nodes)
	#CODE TO FIND THE EXACT NUMBER OF SINGLETONS (NO FOLLOWER/FOLLOWEE USERS)	
	...
	if(rt_found):
		for i in range(1, len(u1)):
			...
			if timestamp1 > timestamp:
				break
			user2 = int(u1[i][u1[i].rfind(',')+1 : ])
			author = int(u1[i][u1[i].index(',')+1:u1[i].rfind(',')])
			numRT = numRT + 1
			author = m[author]
			try:
				user2 = m[user2]
	...
	if(mention_found):
		for i in range(1, len(u2)):
			...
			if timestamp1 > timestamp:
				break
			user2 = int(u2[i][u2[i].rfind(',')+1 : ])
			author = int(u2[i][u2[i].index(',')+1:u2[i].rfind(',')])
	...
	try:
		...
		expose2 = (expose_col[l] - expose_col[l-49])*36000000/(timestamp_col[l] - timestamp_col[l-49])
