#Python file to create the features related to geography #correction for self-initiated adopters and addition of tweeting entropy, number of adopters of followers
import json
import sys
import time
import datetime
			...
			if author not in nodes:
				...
				for value in nodes:
					...
					if value in adj[author]:
						...
						if(buckets[author] != buckets[value]):
							numEdgesCrossing = numEdgesCrossing + 1
				if freshGeo and booladopt:#check for self-initiated, should be true
					selfInitGeo = selfInitGeo + 1
				if booladopt:
					globalAdopters = globalAdopters + 1
					exposureGA = exposureGA + len(adj[author])
				globalFollowerAdopters_set.update(adj[author])
