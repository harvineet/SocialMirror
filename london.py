...
arr = ["/twitterSimulations/timeline_data/anirban/london_fol.txt"]
...
m = dict()
fr = open("/twitterSimulations/timeline_data/anirban/map_info_london.txt")
for line in fr:
	line = line.rstrip()
	u = line.split('\t')
	m[int(u[0])] = int(u[1])
...
num_com = [0]*38110
fr = open('/twitterSimulations/timeline_data/anirban/Infomap-0.15.6/output/london_fol_infomap.tree', 'r')
fr.readline()
for line in fr:
	line = line.rstrip()
	u = line.split(' ')
	...
	try:
		...
		num_com[int(u[0][0:u[0].find(':')])] = num_com[int(u[0][0:u[0].find(':')])] + 1
	except:
		a = 1
fr.close()
...
fr = open('timeline_weng_london', 'r')
fr1 = open('retweeteds_weng_london', 'r')
fr2 = open('user_mentions_weng_london', 'r')
fd = open("feature_london.csv", 'w')
...
for line in fr:
	...
	for i in range(1, len(u)):
		...
		numTweets1 = i
		if(timestamp - int(u[1][0:u[1].index(',')]) > 9000 and numTweets1 > 150):
			break
	...
	val = int(u[150][0:u[150].index(',')]) - int(u[1][0:u[1].index(',')])
