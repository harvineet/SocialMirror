#calculate fmeasure given the prediction probability and true label for classification runs on the dataset after cross validation using WFVP and CGNP features
#
import numpy as np
...
y_predp_ec = []
y_predp_enc = []
fd=open('fmdump1.txt','wb')
with open('en.csv') as fw, open('enc.csv') as fc:
	next(fw)
	next(fc)
	for line in fw:
		line=line.rstrip()
...
y_pred_ec=[1 if x>0.3 else 0 for x in y_predp_ec]
y_pred_enc=[1 if x>0.3 else 0 for x in y_predp_enc]
...
for i in range(runs):
	...
	a_pred=[1 if x>0.3 else 0 for x in a_run]
	b_pred=[1 if x>0.3 else 0 for x in b_run]
