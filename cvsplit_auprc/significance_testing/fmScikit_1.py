...
y_predp_ec = []
y_predp_enc = []
fd=open('fmdump_1.txt','wb')
with open('e.csv') as fw, open('eg.csv') as fc:
	next(fw)
	next(fc)
	for line in fw:
		line=line.rstrip()
...
y_pred_ec=[1 if x>0.4 else 0 for x in y_predp_ec]
y_pred_enc=[1 if x>0.4 else 0 for x in y_predp_enc]
...
for i in range(runs):
	...
	a_pred=[1 if x>0.4 else 0 for x in a_run]
	b_pred=[1 if x>0.4 else 0 for x in b_run]
