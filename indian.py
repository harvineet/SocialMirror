...
arr = ["/twitterSimulations/timeline_data/anirban/indian_fol.txt"]
...
m = dict()
fr = open("/twitterSimulations/timeline_data/anirban/map_info_india.txt")
for line in fr:
	line = line.rstrip()
	u = line.split('\t')
	m[int(u[0])] = int(u[1])
...
num_com = [0]*10800
fr = open('/twitterSimulations/timeline_data/anirban/Infomap-0.15.6/output/indian_fol_infomap.tree', 'r')
fr.readline()
for line in fr:
	line = line.rstrip()
	u = line.split(' ')
...
fr = open('timeline_weng_india', 'r')
fr1 = open('retweeteds_weng_india', 'r')
fr2 = open('user_mentions_weng_india', 'r')
fd = open("feature_india.csv", 'w')
...
for line in fr:
	...
	for i in range(1, len(u)):
		...
		numTweets1 = i
		if(timestamp - int(u[1][0:u[1].index(',')]) > 9000 and numTweets1 > 100):
			break
	...
	val = int(u[100][0:u[100].index(',')]) - int(u[1][0:u[1].index(',')])
