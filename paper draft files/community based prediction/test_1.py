...
with open(topic_fname) as f:
	for line in f:
		...
		# min_time = min(min_time,min(map(int,timestamp_hashtag)))
		...
		for a in early_adopters:
			if a not in deleted_nodes:
				try:
					inf_comm_adopters += node_community_map[a]
				except:
					non_network_user.add(a)
					continue
		for t in early_tweets:
			if t not in deleted_nodes:
				try:
					inf_comm_tweets += node_community_map[t]
				except:
					non_network_user.add(a)
					continue
		...
		last_early_tweet_time = int(timestamp_hashtag[early_tweet_threshold-1])
		...
		if men_tag != []:
			for (t,u1,u2) in men_tag:
				if(int(t)<=last_early_tweet_time):
					try:
...
print len(non_network_user), count
