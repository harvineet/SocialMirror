...
# retweet and mention file open for each iteration of number of tweets, correct if hashtags are in same order in timeline, rt, mention files, alphabetically ordered in all three files, check assumption of same hashtag order in rt, men with timeline_weng, to avoid reading file again
import json
import sys
import time
import datetime
...
for line in fr:
	line = line.rstrip()
	u = line.split(' ')
	fd=open("./feature_plot/feature_"+u[0]+".csv",'w')
	fd.write("TweetNum,TagName,RatioSecondtoFirst,RatioSelfInitCommu,RatioCrossGeoEdges,#globalAdoptersFollowers,#globalAdopters,#heavyusers,Density,LargestSize,NumEdges,Conduct1,Conduct2,Conduct3,Conduct4,Conduct2d,NumTweets,TimeFirst1000,NoOfAdopters,Conductance,RatioOfSingletons,RatioOfConnectedComponents,InfectedCommunities,UsageEntropy,NumOfRT,NumOfMention,IntraRT,IntraMen,Class\n")
	step_size=50
	t_limit = min(len(u),10100)
	for pt in range(50,t_limit,step_size):
