...
# retweet_line = "";
# u1 = []
for pred_thr in [1500,2000,2500]:#200,250,500,750,1000,
	fr = open('timeline_data/timeline_weng', 'r')
	fr1 = open('retweeteds_weng', 'r')
	fr2 = open('user_mentions_weng', 'r')
	fd=open("./conductance_std_features/feature_"+str(pred_thr)+"_no5hr.csv",'w')
	fd_cond=open("./conductance_std_features/cond_values_all/feature_"+str(pred_thr)+"_no5hr.csv",'w')
	...
	for line in fr:
		...
		for i in reversed(range(10)):
			try:
				expose1 = (expose_col[l-i] - expose_col[l-i-20])/20 #*36000000/(timestamp_col[l-i] - timestamp_col[l-i-20])
				c_expose1.append(expose1)
				expose2 = (expose_col[l-i] - expose_col[l-i-50])/50#*36000000/(timestamp_col[l-i] - timestamp_col[l-i-50])
				c_expose2.append(expose2)
				expose3 = (expose_col[l-i] - expose_col[l-i-100])/100#*36000000/(timestamp_col[l-i] - timestamp_col[l-i-100])
				c_expose3.append(expose3)
				if(pred_thr==200 or pred_thr==250):
					expose4 = (expose_col[l-i] - expose_col[0])/250#*36000000/(timestamp_col[l-i] - timestamp_col[0]) # 250 not correct, changes for different i
					c_expose4.append(expose4)
				else:
					expose4 = (expose_col[l-i] - expose_col[l-i-250])/250#*36000000/(timestamp_col[l-i] - timestamp_col[l-i-250])
					c_expose4.append(expose4)
				#expose5 = (expose_col[l] - expose_col[l-500])*36000000/(timestamp_col[l] - timestamp_col[l-500])
				pexpose2 = (expose_col[l-i-50] - expose_col[l-i-100])/50#*36000000/(timestamp_col[l-i-50] - timestamp_col[l-i-100])
				expose2d = (expose2 - pexpose2)/50#*36000000/(timestamp_col[l-i] - timestamp_col[l-i-50])
		...
		if len(u) > 10000:
			...
			for i in range(0, len(timestamp_col)):
				fdtemp.write(str(timestamp_col[i]) + ' ' + str(expose_col[i]) + '\n')
			fdtemp.close()'''
			fd.write(str(u[0])+','+str(float(secondLarge)/large)+','+str(selfInitGeo/infectCommunit)+','+str(numEdgesCrossing/numEdges)+','+str(exposureGA)+','+str(globalAdopters)+','+str(heavy)+','+str(float(numEdges)/len(nodes))+','+str(large)+','+str(numEdges)+',' + str(expose1)+','+str(expose2)+','+str(expose3)+','+str(expose4)+','+str(expose2d)+','+str(numTweets)+','+str(val)+','+str(len(nodes))+','+str(float(expose_num)/expose)+','+str(float(singleton)/len(nodes))+','+str(float(numOfSets)/len(nodes))+','+str(infectCommunit)+','+str(usagee)+','+str(numRT)+','+str(numMention)+','+str(val1)+','+str(val2)+','+str(stdev(c_expose1))+','+str(stdev(c_expose2))+','+str(stdev(c_expose3))+','+str(stdev(c_expose4))+','+str(stdev(c_expose2d))+','+str(stdev(c_cond))+',0\n')
